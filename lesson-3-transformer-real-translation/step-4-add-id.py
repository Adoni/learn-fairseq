...
def run():
    base_dir = "./iwslt14"
    src_dict = MyTranslationTask.build_dictionary(f"{base_dir}/bpe/train.json", "src")
    tgt_dict = MyTranslationTask.build_dictionary(f"{base_dir}/bpe/train.json", "tgt")
    save_dict(src_dict, tgt_dict, base_dir)
    print("完成字典存储")

    for split in tqdm(["train", "valid", "test"]):
        input_file_path = f"{base_dir}/bpe/{split}.json"
